...
- (void)viewDidLoad {
    ...
    self.screenName = @"Filter Overview";
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [self.allFilterItems count];
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return (self.view.frame.size.height - 211) / [self.allFilterItems count];
}
...
- (NSString *)detailStringForFilterItems:(NSString *) key {
    ...
    for(NSString *filterName in [[filterItems allKeys] sortedArrayUsingSelector:@selector(localizedCompare:)]) {
